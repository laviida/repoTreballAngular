...
export class ConversationalFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    dispatcher.addEventListener("cf-flow-update", function (event) {
      ...
      this.cf = ConversationalForm.startTheConversation({
        ...
        options: {
          ...
          submitCallback: () => {
            ...
            this.tournamentService.initializeTournament({ name: formData.name, mode: formData['mode'][0], num_teams: parseInt(formData.num_teams[0]), type: formData.type[0] }, formData.teams, this.teams);
            //this.router.navigate(["/tournaments"]);
            this.cf.addRobotChatResponse('Torneo creado correctamente.');
          },
          flowStepCallback: async (dto: any, success: Function, error: Function) => {
            let data = this.cf.getFormData(true);
            if (dto.tag.name == 'type' && dto.tag.value[0] == TournamentType.ARBITRARY) this.cf.addTags(this.serviceCf.getTeamsTag(this.teams));
